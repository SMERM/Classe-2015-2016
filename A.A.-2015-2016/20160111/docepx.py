...
def accelerando(t0,t1,w0,w1, freq):
	...
	ap= (p1-p0)/(t1-t0) # coeff angolare del rapporto tra le funz dur e step
	bp= p0(ap*t0) # quota della retta  p
	at= t0 # dove parte la riproduzione e il processo di accellerando
	print ";nota con arg %5.2f %5.2f %5.2f %5.2f" % (t0,t1,w0,w1, freq)
	while (at < (t0+ftime)):
		w= math.exp((aw*at)+bw)  # prima funz lineare (larghezza finestra)
		p= ap*at+bp # seconda funz sul rapporto tra dur e step 
		...
		at= at+w # trigger
	print "i1 %8.4f %8.4f %8.4f" % (at,totdur-ftime, freq) # pezzo continua normalmente dopo il processo di accellerando 
