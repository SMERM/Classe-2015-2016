...
phi= 0.6180339
def accelerando_gliss(t0,t1,w0,w1,freq0,freq1):
	totdur= t1-t0
	...
	while (at < (t0+ftime)):
		...
		at= at+w
	finaldur= at+dur
	c=totdur/finaldur # fattore moltiplicativo per la sommatoria dei segmenti (per forzare la retta)
	print ";nota con arg %5.2f %5.2f %5.2f %5.2f %5.2f %5.2f" % (t0, t1, w0, w1, freq0, freq1)
	print ";valori attesi: w0=%8.5f, w1=%8.5f" % (math.exp(bw), math.exp(ftime*aw+bw))
	print ";valori finali frequenza: freqa=%8.5f, freqb=%8.5f" % (freqb, freqa*ftime+freqb)
	print ";coeff c=%8.5f" % (c)
	at=t0
	while (at < (t0+ftime)):
		atstart= at-t0
		w= math.exp((aw*atstart)+bw)*c # prima funzione lineare (larghezza cella)
		...
		print "i1 %8.4f %8.4f %8.4f %8.4f; w= %7.4f, p= %7.4f, step= %7.4f" % (at, dur, freqstart, freqend, w, p, step) 
		at= at+w
	print "i1 %8.4f %8.4f %8.4f %8.4f" % (at, totdur-ftime, freq1, freq1)
